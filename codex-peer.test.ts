...
// findHostSessionPid, not findSessionPid: after reconciling the two branches the walk keeps two
// named policies rather than one overloaded function. findSessionPid answers "which Claude session"
// and is deliberately claude-only; findHostSessionPid answers "which host session, whichever host"
// and is what server.ts calls. These tests are about the host question.
import { findHostSessionPid } from "./spool";
import {
  cleanupAll,
  reserveFreePort,
  sweepBrokerOnPort,
...
// resolving nothing. Cleared in this process (findHostSessionPid reads it here, in-process) and omitted
...
test("a session under a codex ancestor resolves that session, not a claude one further up", async () => {
  ...
  expect(findHostSessionPid(childPid)).toBe(shim.pid);
}, 20_000);
...
test("a codex ancestor nested under a claude ancestor wins, because it is nearer", async () => {
  ...
  expect(findHostSessionPid(childPid)).toBe(codexPid);
  expect(findHostSessionPid(childPid)).not.toBe(claudeProc.pid);
}, 20_000);
...
test("no agent ancestor at all resolves to null rather than a guess", async () => {
  ...
  expect(findHostSessionPid(orphanPid)).toBeNull();
}, 20_000);
